...
namespace hippo_control {
    namespace mixer_bluerov {
        ...
        void SimpleMixer::InitializeMixerMatrix(double alpha_f, double alpha_r, double l_hf, double l_hr, double l_vx,
                                                double l_vy) {
          ...
          for (int i = 0; i < kOutputHorizontal; i++) {
            for (int j = 0; j < InputChannels::kInputHorizontal; j++) {
              mixer_matrix_inverse_horizontal_(i, j) = mixer_matrix_inverse_(kOutputIdxsHorizontal[i],
                                                                             InputChannels::kIdxsHorizontal[j]);
            }
          }
          for (int i = 0; i < kOutputVertical; i++) {
            for (int j = 0; j < InputChannels::kInputVertical; j++) {
              mixer_matrix_inverse_vertical_(i, j) = mixer_matrix_inverse_(kOutputIdxsVertical[i],
                                                                           InputChannels::kIdxsVertical[j]);
            }
          }
        ...
        void SimpleMixer::UpdateMinima() {
          ...
          for (ThrusterModel &model: thruster_models_) {
            if (model.quadratic_coefficient > 0) {
              extremum =
                      -model.linear_coefficient /
                      std::max(model.quadratic_coefficient, eps_); // assumes that parabola is opened on top
            } else if (model.quadratic_coefficient == 0.0) {
              extremum = 0.0;
            } else {
              std::cout << "Expected quadratic coefficient greater or equal zero, but got: "
                        << model.quadratic_coefficient
                        << std::endl;
              return;
            }
            if (extremum > 0.0) {
        ...
        bool SimpleMixer::IsInDeadZone(const double &thruster_value, const double &deadzone_min,
        ...
        void SimpleMixer::UpdateScaling(const Eigen::Matrix<double, size_output, 1> &thrusters0, double deadzone_min,
                                        double deadzone_max, const Eigen::Matrix<double, size_output, 1> &nullspace,
                                        double &scaling) {
          for (int i = 0; i < size_output; i++) {
            if (!IsInDeadZone(thrusters0(i) + scaling * nullspace(i), deadzone_min, deadzone_max)) {
              continue;
            }
            if (((deadzone_min - thrusters0(i)) / nullspace(i) * scaling > 0)
              ...
              scaling = (deadzone_max - thrusters0(i)) / nullspace(i) + eps_ * scaling /
                                                                        abs(scaling);
        ...
        template<int size_output>
        bool SimpleMixer::IsSaturated(const Eigen::Matrix<double, size_output, 1> &thrusters, double limit_min,
                                      double limit_max) {
          for (int i = 0; i < size_output; i++) {
        ...
        template<int size_output>
        Eigen::Matrix<double, size_output, 1>
        SimpleMixer::ChooseBetter(const std::array<Eigen::Matrix<double, size_output, 1>, 2> &thrusters_scaled,
                                  const Eigen::Matrix<double, size_output, 1> &last_thrusters) {
          double cost_0 =
                  thrusters_scaled[0].norm() + weighting_last_input_ * (thrusters_scaled[0] - last_thrusters).norm();
          double cost_1 =
                  thrusters_scaled[1].norm() + weighting_last_input_ * (thrusters_scaled[1] - last_thrusters).norm();
          if (cost_0 <= cost_1) {
        ...
        Eigen::Matrix<double, size_output, 1>
        SimpleMixer::ResolveDeadzone(const Eigen::Matrix<double, size_output, size_input> &M_inv,
                                      const Eigen::Matrix<double, size_output, 1> &nullspace_vector,
                                      double deadzone_min, double deadzone_max, double limit_min, double limit_max,
          ...
          for (int i = 0; i < 2; i++) {
            ...
            while (thrusters_in_deadzone) {
              UpdateScaling(thrusters, deadzone_min, deadzone_max, nullspace_vector, scalings[i]);
        ...
        double SimpleMixer::ApplyInput(
          ...
          double limit_max = scaling_saturation_limit_up_ *
                             RevsPerSecToThrust(thruster_models_[ThrustDirection::forward], max_rotations_per_second_);  // positive
          ...
          if (abs_thrust_wrench < InputChannels::kCount * zero_throttle_threshold_) {
            ...
            output_vec_horizontal = ResolveDeadzone(mixer_matrix_inverse_horizontal_, nullspace_horizontal_,
            ...
            output_vec_vertical = ResolveDeadzone(mixer_matrix_inverse_vertical_, nullspace_vertical_,
                                                   -thruster_models_[ThrustDirection::backward].deadzone_minimum,
                                                   thruster_models_[ThrustDirection::forward].deadzone_minimum,
