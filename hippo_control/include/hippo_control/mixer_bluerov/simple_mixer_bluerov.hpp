...
static constexpr std::array<int, kOutputHorizontal> kOutputIdxsHorizontal = {0, 1, 2, 3};
static constexpr std::array<int, kOutputVertical> kOutputIdxsVertical = {4, 5, 6, 7};
...
class SimpleMixer {
 public:
  ...
  void InitializeMixerMatrix(double alpha_f, double alpha_r, double l_hf, double l_hr, double l_vx, double l_vy);
  ...
  void SetDeadzoneMinimum(double _v, int idx) { thruster_models_[idx].deadzone_minimum = _v; }
  inline double DeadzoneMinimum(int idx) const { return thruster_models_[idx].deadzone_minimum; }
  ...
  void SetCompensateDeadZone(bool compensate_deadzone){compensate_deadzone_ = compensate_deadzone;}
  void SetWeightingLastInput(double _v){weighting_last_input_ = _v;}
  void SetScalingSaturationLimitLow(double _v){scaling_saturation_limit_low_ = _v;}
  void SetScalingSaturationLimitUp(double _v){scaling_saturation_limit_up_ = _v;}
 ...
 private:
  ...
  void IsInDeadzone(const Eigen::Matrix<double, size_output, 1> &thrusters0, double deadzone_min, double deadzone_max, const Eigen::Matrix<double, size_output, 1> &nullspace, const double &scaling, double &scaling_out, ScalingStatus &scaling_status);

  template <int size_output>
  void IsSaturated(const Eigen::Matrix<double, size_output, 1> &thrusters, double limit_min, double limit_max, ScalingStatus &scaling_status);

  template <int size_output>
  Eigen::Matrix<double, size_output, 1> ChooseBetter(const std::array<Eigen::Matrix<double, size_output, 1>, 2> &thrusters_scaled, const Eigen::Matrix<double, size_output, 1> &last_thrusters);

  template <int size_input, int size_output>
  Eigen::Matrix<double, size_output, 1> ResolveDeadzone(const Eigen::Matrix<double, size_output, size_input> &M_inv, const Eigen::Matrix<double, size_output, 1> &nullspace_vector,
  ...
  Eigen::Matrix<double, kOutputHorizontal, InputChannels::kInputHorizontal> mixer_matrix_inverse_horizontal_;
  Eigen::Matrix<double, kOutputVertical, InputChannels::kInputVertical> mixer_matrix_inverse_vertical_;
  Eigen::Matrix<double, kOutputHorizontal, 1> nullspace_horizontal_;
  Eigen::Matrix<double, kOutputVertical, 1> nullspace_vertical_;
  Eigen::Matrix<double, kOutputHorizontal, 1> last_output_horizontal_{0.0, 0.0, 0.0, 0.0};
  Eigen::Matrix<double, kOutputVertical, 1> last_output_vertical_{0.0, 0.0, 0.0, 0.0};
